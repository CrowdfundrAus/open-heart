...
function jw_option($index1, $index2 = false) {
    global $smof_data;
    if ($index2) {
...
function get_metabox($name) {
    global $post;
    if ($post) {
        $metabox = get_post_meta($post->ID, 'designinvento_' . strtolower(THEMENAME) . '_options', true);
...
function set_metabox($name, $val) {
    ...
    if ($post) {
        $metabox = get_post_meta($post->ID, 'designinvento_' . strtolower(THEMENAME) . '_options', true);
        $metabox[$name] = $val;
        return update_post_meta($post->ID, 'designinvento_' . strtolower(THEMENAME) . '_options', $metabox);
    }
    return false;
}
...
function theme_menu() {	
    ...
    if($nav_menu) {
        wp_nav_menu(array(
            'container' => 'div',
            'container_class' => 'jw-menu-container',
            'menu' => $nav_menu,
            'menu_id' => 'menu',
            'menu_class' => 'sf-menu',
            'fallback_cb' => 'no_main')
...
function no_main() {
    echo "<div class='jw-menu-container clearfix'><ul id='menu' class='sf-menu'>";
...
//=======================================================
function theme_logo() {
    $top = jw_option('logo_top') != "" ? (' padding-top:' . jw_option('logo_top') . 'px;') : '';
    $bottom = jw_option('logo_bottom') != "" ? (' padding-bottom:' . jw_option('logo_bottom') . 'px;') : '';
    $bgcolor = jw_option('logo_bg') ? '' : (' background-color: transparent;');
    echo '<div class="jw-logo" style="' . $top . $bottom . $bgcolor . '">';
    echo '<a class="logo" href="' . home_url() . '">';
    if (jw_option("theme_logo") == "") {
        bloginfo('name');
    } else {
        if (jw_option("logo_retina"))
            echo '<img class="logo-img" src="' . jw_option("theme_logo_retina") . '" style="width:' . jw_option('logo_width') . 'px" alt="' . get_bloginfo('name') . '"/>';
        else
            echo '<img class="logo-img" src="' . jw_option("theme_logo") . '" alt="' . get_bloginfo('name') . '"/>';
    }
    echo '</a>';
    echo '</div>';
...
function get_featuredtext() {
    ...
    if (is_singular()) {
        ...
        $return .= (jw_option('pt_category') ? jw_option('pt_category') : __("Category", "designinvento")) . " : " . single_cat_title("", false);
        $return .= "</h1>";
        return $return;
    } elseif (is_tax('cat_portfolio')) {
        $return = "<h1 class='page-title'>";
        $return .= (jw_option('pt_portfolio') ? jw_option('pt_portfolio') : __("Cause", "designinvento")) . " : " . single_cat_title("", false);
        $return .= "</h1>";
        return $return;
        ...
        $return = "<h1 class='page-title'>";
        $return .= (jw_option('pt_tag') ? jw_option('pt_tag') : __("Tag", "designinvento")) . " : " . single_tag_title("", false);
        $return .= "</h1>";
        return $return;
    } elseif (is_404()) {
        $return = "<h1 class='page-title'>" . jw_option('pt_nothing_found') ? jw_option('pt_nothing_found') : __("Nothing Found!", "designinvento") . "</h1>";
        return $return;
    } elseif (is_author()) {
        global $author;
        $userdata = get_userdata($author);
        $return = "<h1 class='page-title'>" . (jw_option('pt_author') ? jw_option('pt_author') : __("Author: ", "designinvento")) . $userdata->display_name . "</h1>";
        return $return;
    } elseif (is_archive()) {
        $return = "<h1 class='page-title'>";
        if (is_day()) {
            $return .= (jw_option('pt_daily_arch') ? jw_option('pt_daily_arch') : __("Daily Archives", "designinvento")) . " : " . get_the_date();
        } elseif (is_month()) {
            $return .=(jw_option('pt_monthly_arch') ? jw_option('pt_monthly_arch') : __("Monthly Archives", "designinvento")) . " : " . get_the_date("F Y");
        } elseif (is_year()) {
            $return .= (jw_option('pt_yearly_arch') ? jw_option('pt_yearly_arch') : __("Yearly Archives", "designinvento")) . " : " . get_the_date("Y");
        } else {
            $return .= jw_option('pt_blog_arch') ? jw_option('pt_blog_arch') : __("Blog Archives", "designinvento");
        }
        $return .= "</h1>";
        return $return;
    } elseif (is_search()) {
        $return = "<h1 class='page-title'>" . (jw_option('pt_search_result') ? jw_option('pt_search_result') : __("Search results for", "designinvento")) . " : " . get_search_query() . "</h1>";
        return $return;
    }
}
...
function favicon() {
    if (jw_option('theme_favicon') == "") {
        echo '<link rel="shortcut icon" href="' . THEME_DIR . '/assets/img/favicon.ico"/>';
    } else {
        echo '<link rel="shortcut icon" href="' . jw_option('theme_favicon') . '"/>';
    }
    if (jw_option('favicon_retina')) {
        echo jw_option('favicon_iphone') != "" ? ('<link rel="apple-touch-icon" href="' . jw_option('favicon_iphone') . '"/>') : '';
        echo jw_option('favicon_iphone_retina') != "" ? ('<link rel="apple-touch-icon" sizes="114x114" href="' . jw_option('favicon_iphone_retina') . '"/>') : '';
        echo jw_option('favicon_ipad') != "" ? ('<link rel="apple-touch-icon" sizes="72x72" href="' . jw_option('favicon_ipad') . '"/>') : '';
        echo jw_option('favicon_ipad_retina') != "" ? ('<link rel="apple-touch-icon" sizes="144x144" href="' . jw_option('favicon_ipad_retina') . '"/>') : '';
    }
}

function causes_image($width = 290, $height = "", $prettyphoto = true, $prettyVideoURL='') {
...
function events_image($width = 290, $height = "", $prettyphoto = true, $prettyVideoURL='') {
    global $post,$portAtts;
    if (has_post_thumbnail($post->ID)) {
        $portAtts['hide_hover'] =isset($portAtts['hide_hover']) ?$portAtts['hide_hover'] :'false';
        $portAtts['link_type']  =isset($portAtts['link_type'])  ?$portAtts['link_type']  :'view_large';
...
function portfolio_image($width = 290, $height = "", $prettyphoto = true, $prettyVideoURL='') {
    global $post,$portAtts;
    if (has_post_thumbnail($post->ID)) {
        $portAtts['hide_hover'] =isset($portAtts['hide_hover']) ?$portAtts['hide_hover'] :'false';
        $portAtts['link_type']  =isset($portAtts['link_type'])  ?$portAtts['link_type']  :'view_large';
		...
		<?php
			...
			<div class="cause-overlay"></div> 
												...
												<figcaption>
													...
													<?php
													$cat = $terms = wp_get_post_terms( $post->ID, 'cat_portfolio', '' );
													?>
													<p><?php echo esc_attr($cat[0]->name); ?></p>
													
													<a class="fancybox" href="<?php echo get_metabox('preview_url');?>" target="_blank">
													</a>
...
function portfolio_image_single($width = 290, $height = "", $prettyphoto = true, $prettyVideoURL='') {
    global $post,$portAtts;
    if (has_post_thumbnail($post->ID)) {
        $portAtts['hide_hover'] =isset($portAtts['hide_hover']) ?$portAtts['hide_hover'] :'false';
        $portAtts['link_type']  =isset($portAtts['link_type'])  ?$portAtts['link_type']  :'view_large';
...
function portfolio_gallery($width = 290, $height = "", $ids = "", $prettyphoto = true) {
	global $post;
    if (!empty($ids)) {
        ?>
        <div class="gallery-container clearfix">
			...
			<a href="<?php echo get_permalink( $post->ID ); ?> " class="gallery-lin"></a>	
        </div>    
        <?php
    }
}
...
function format_standard() {
    ...
    if (has_post_thumbnail($post->ID)) {
        ...
        $link = get_permalink();
        $rell = '';
        if (get_metabox("layout") == "full" && is_single()){
			$width = 1170;
		 }else{
		 $width = 870;
		 }
        ...
        <div class="loop-image">
				...
				<div class="image-overlay">
					<a href="<?php echo esc_url($link); ?>"<?php echo wp_kses_post($rell); ?> title="<?php echo the_title(); ?>"></a>		
				</div>
        </div>
        <?php
    }
...
function format_video() {
	...
	if (empty($video_thumb)) {
		if (has_post_thumbnail($post->ID)) {        
			...
			//$video_thumb = post_image_show($width, $height);	  
			$video_thumb =  wp_get_attachment_url( get_post_thumbnail_id($post->ID) );
			echo  '<style>.jp-jplayer-video img{width:'.$width.'px;height:'.$height.'px !important;}</style>';
    ...
    if (!empty($video_embed)) {
        ...
        <div id="jquery_jplayer_<?php echo esc_attr($post->ID); ?>" class="jp-jplayer jp-jplayer-video" data-pid="<?php echo esc_attr($post->ID); ?>" data-m4v="<?php echo esc_url($video_m4v); ?>" data-thumb="<?php echo wp_kses_post($video_thumb); ?>"></div>
        <?php if (is_single()) { ?>
		
		<div class="isotope-item">			
			<div class="jp-type-single">
				<div id="jp_interface_<?php echo esc_attr($post->ID); ?>" class="jp-interface" style="top:<?php echo esc_attr($height) /2 - 25; ?>px">
					<ul class="jp-controls">
					   <li><a href="#" class="jp-play" tabindex="1">play</a></li>
					   <li><a href="#" class="jp-pause" tabindex="1" >pause</a></li>	
					   <li><a href="#" class="jp-mute" tabindex="1" style="top:<?php echo esc_attr($height) /2; ?>px">mute</a></li>
                       <li><a href="#" class="jp-unmute" tabindex="1" style="top:<?php echo esc_attr($height) /2; ?>px">unmute</a></li>
					</ul>
					<div class="jp-progress-container" style="top:-<?php echo esc_attr($height) /2 - 25; ?>px">
                        <div class="jp-progress">
                            <div class="jp-seek-bar">
                                <div class="jp-play-bar"></div>
                            </div>
...
function pagination() {
    ...
    if (1 != $pages) {
        ...
        echo "<div class='jw-pagination pagination'>";
        ...
        $pagination = paginate_links(
                array(
                    ...
                    'type' => 'list',
                    'prev_text' => __('&laquo;', 'designinvento'),
                    'next_text' => __('&raquo;', 'designinvento'),
        ));
...
if (!function_exists('infinite')) {
    ...
    function infinite() {
        ...
        if (1 != $pages) {
            ...
            echo '<a class="btn btn-small no-more" href="#">' . __('No more posts', 'designinvento') . '</a>';
            echo '<a class="btn btn-small loading" href="#">' . __('Loading posts ...', 'designinvento') . '</a>';
            echo '<a class="btn btn-small next" href="' . get_pagenum_link($paged + 1) . '"><i class="icon-repeat"></i>' . __('Load More Items', 'designinvento') . '</a>';
            echo '</div>';
        }
    }
...
//        } else {
        ...
        if ($comment_count == 0) {
            $comment_trans = __('00 Comments', 'designinvento');
        } elseif ($comment_count == 1) {
            $comment_trans = __('01 Comments', 'designinvento');
        } else {
            $comment_trans = $comment_count . ' Comments' . __('', 'designinvento');
        }
        return "<a href='" . get_comments_link() . "' title='" . $comment_trans . "' class='comment-count'>" . $comment_trans . "</a>";
//        }
    }
}
function comment_count_isotop() {
...
//        } else {
        ...
        if ($comment_count == 0) {
            ...
            $comment_trans = '0'.$comment_count . ' Comments';
        }else {
            $comment_trans = $comment_count . ' Comments ' . __('', 'designinvento');
        }
        return "<a href='" . get_comments_link() . "' title='" . $comment_trans . "' class='comment-count'>" . $comment_trans . "</a>";
            ...
            <div class="comment-author"><span class="reply-line"></span>
                ...
                <div class="comment-meta">
                    <span class="comment-author-link">
                        <?php echo __('', 'designinvento') . " " . get_comment_author_link() . " "; ?>
                    </span>                            
                    
                    <span class="comment-replay-link"><i class="fa fa-angle-double-right"></i><?php comment_reply_link(array_merge($args, array('depth' => $depth, 'max_depth' => $args['max_depth']))) ?></span>
                </div>
            </div>
...
if (!function_exists('jw_comment_form')) {

    function jw_comment_form($fields) {
        global $id, $post_id;
        if (null === $post_id)
            $post_id = $id;
        else
        ...
        $fields = array(
            ...
            '<input placeholder="' . __('Name', 'designinvento') . '" id="author" name="author" type="text" value="' . esc_attr($commenter['comment_author']) . '" size="30"' . $aria_req . ' /></p>',
            'email' => '<p class="comment-form-email">' .
            '<input placeholder="' . __('Email', 'designinvento') . '" id="email" name="email" type="text" value="' . esc_attr($commenter['comment_author_email']) . '" size="30"' . $aria_req . ' /></p></div>'
        );
        return $fields;
    }

    add_filter('comment_form_default_fields', 'jw_comment_form');
}
    ...
    function about_author() {
        ...
        if ($jw_author) {
			...
			<div class="about-author">
				<div class="jw-author clearfix">
					<div class="author-image"><?php
						$jw_author_email = get_the_author_meta('email');
						echo get_avatar($jw_author_email, $size = '70');
						?>
					</div>
					<div class="author-title">Written By: <?php
						if (is_author())
							the_author();
						else
					...
					<p><?php
						...
						if ($description != '')
							...
							_e('The author didnt add any Information to his profile yet', 'designinvento');
						?>
					</p>
				</div>
			</div><?php
...
function related_causes() {
    global $post, $jw_options, $portAtts;

    $tags = wp_get_post_terms($post->ID, 'cat_cause', array("fields" => "ids"));

    if ($tags) {
        $rel_title = jw_option('translate_relatedcause') ? jw_option('translate_relatedcause') : __('RELATED CAUSE ITEMS', 'designinvento');
        echo do_shortcode('[jw_item_title title="' . $rel_title . '"]');
        $tag_ids = "";
        foreach ($tags as $tag)
            $tag_ids .= $tag . ",";
        ...
        query_posts($query);
        $jw_options['pagination'] = 'none';
        $portAtts['hide_favorites'] = jw_option('hide_favorites')?'true':'false';
        echo '<div class="row"><div class="related_causes"><div class="jw_cause">';
        get_template_part("loop", "cause");
        echo '</div></div></div>';
    }
}
...
function related_events() {
    ...
    $tags = wp_get_post_terms($post->ID, 'cat_event', array("fields" => "ids"));

    if ($tags) {
        $rel_title = jw_option('translate_relatedevent') ? jw_option('translate_relatedevent') : __('RELATED EVENT ITEMS', 'designinvento');
        echo do_shortcode('[jw_item_title title="' . $rel_title . '"]');
        $tag_ids = "";
        foreach ($tags as $tag)
            $tag_ids .= $tag . ",";
...
function related_portfolios() {
    ...
    if ($tags) {
        $rel_title = jw_option('translate_relatedportfolio') ? jw_option('translate_relatedportfolio') : __('RELATED GALLERY', 'themeinvento');
        echo do_shortcode('[jw_item_title title="' . $rel_title . '"]');
        $tag_ids = "";
        foreach ($tags as $tag)
            $tag_ids .= $tag . ",";
        ...
        $jw_options['pagination'] = 'none';
        $portAtts['hide_favorites'] = jw_option('hide_favorites')?'true':'false';
        echo '<div class="row"><div class="span12 related_portfolios"><div class="jw-portfolio">';
...
function jw_social() {
    ...
    foreach ($jw_socials as $key => $social) {
        if (jw_option($social['name']) != "") {
            echo '<a href="' . str_replace('*', jw_option($social['name']), $social['link']) . '" target="_blank" title="' . $key . '" class="' . $key . '"><span class="jw-icon-' . $key . '"></span></a>';
        }
    }
}
