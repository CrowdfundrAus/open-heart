...
if (!function_exists('pagebuilder_admin_scripts')) {
    function pagebuilder_admin_scripts() {
        ...
        if (current_user_can('edit_posts') && ($pagenow == 'post-new.php' || $pagenow == 'post.php') && ($post_type==='page'||$post_type==='post'||$post_type==='jw_cause'||$post_type==='jw_portfolio'||$post_type==='jw_event'||$post_type==='jw_price'||$post_type==='jw_testimonial'||$post_type==='jw_team')) {
            wp_register_script('post-pagebuilder', THEME_DIR . '/inc/assets/js/pagebuilder.js');
            wp_register_script('easy-pie-chart', THEME_DIR . '/assets/js/jquery.easy-pie-chart.js');
           wp_register_script('jquery-ui-dialog', 'http://code.jquery.com/ui/1.10.4/jquery-ui.js');            wp_enqueue_script('post-pagebuilder');            wp_enqueue_script('easy-pie-chart');			wp_enqueue_script('jquery-ui-dialog');
        }
    }
}
if (!function_exists('pagebuilder_admin_styles')) {
    function pagebuilder_admin_styles() {
        global $pagenow,$post_type;
        if (current_user_can('edit_posts') && ($pagenow == 'post-new.php' || $pagenow == 'post.php') && ($post_type==='page'||$post_type==='post'||$post_type==='jw_cause'||$post_type==='jw_portfolio'||$post_type==='jw_event'||$post_type==='jw_price'||$post_type==='jw_testimonial'||$post_type==='jw_team')) {
            wp_register_style('post-pagebuilder', THEME_DIR . '/inc/assets/css/pagebuilder.css', false, '1.00', 'screen');
...
//====== START - Globals  ======
global $jw_pbItems, $jw_pbHeadSettings, $jw_pbRowSettings;
//Background position
$bgposition = array(
    "left top" => "left top",
...
//causes Catigories
$cause = get_terms('cat_cause', 'hide_empty=0');
$cause_categories = array("0" => "Select Category");
if(!empty($cause)) {
    foreach ($cause as $cause) {
        $cause_categories["$cause->term_id"] = $cause->name;
    }
}
//Portfolio Catigories
$portfolio = get_terms('cat_portfolio', 'hide_empty=0');
$portfolio_categories = array("0" => "Select Category");
if(!empty($portfolio)) {
    foreach ($portfolio as $gallery) {
        $portfolio_categories["$gallery->term_id"] = $gallery->name;
    }
}
//Portfolio Catigories
$event = get_terms('cat_event', 'hide_empty=0');
$event_categories = array("0" => "Select Category");
if(!empty($event)) {
    foreach ($event as $event) {
        $event_categories["$event->term_id"] = $event->name;
    }
}
...
//includes
require_once (THEME_PATH . "/inc/pagebuilder/elements.php");
require_once (THEME_PATH . "/inc/pagebuilder/pagebuilder_render.php");
//====== END   - Globals   ======
//====== START - Functions ======
...
if (!function_exists('getItemField')) {
    function getItemField($itemSlug, $itemArray) {
        ...
        $class = 'field'; ?>
        <div class="field-item<?php echo esc_attr($type) === 'hidden' ? ' hidden' : ''; echo' type-' . $type; echo wp_kses_post($tinyMCE)?' editor':''; echo " ".$itemSlug; ?>"><?php
            if($type!='container'){
            ...
            <div class="field-data"><?php
                switch ($type) {
                    case 'cc' : { ?>
                        <div class="button show-cc-modal"><?php _e('Edit Chart','ewebcraft'); ?></div>
                        <div class="cc-viewer" style="padding: 20px 0;"></div><?php
                        break;
                    }
                    case 'fa' : { ?>
                        <div class="button show-fa-modal"><?php _e('Edit Icon','ewebcraft'); ?></div>
                    ...
                    case 'button':
                    case 'text' : { ?>
                        <input    data-name="<?php echo esc_attr($itemSlug); ?>" data-type="<?php echo esc_attr($type); ?>" class="<?php echo esc_attr($class); ?>" value="<?php echo htmlspecialchars($default); ?>" placeholder="<?php echo esc_attr($holder); ?>" data-selector="<?php echo esc_attr($selector); ?>" data-save-to="<?php echo esc_attr($save_to); ?>" type="<?php echo esc_attr($type); ?>" /><?php
                        if (!empty($itemArray['data'])) {
                            global $jw_pbItems;
                            echo '<div class="data hidden">';
                            $tmpItem = $itemArray['data']['item'];
                            $tmpSettings = $itemArray['data']['settings'];
                            getItemField($tmpSettings, $jw_pbItems[$tmpItem]['settings'][$tmpSettings]);
                            echo '</div>';
                        }
                        break;
                    }
                    case 'color': { ?>
                        ...
                        <input    data-name="<?php echo esc_attr($itemSlug); ?>" data-type="<?php echo esc_attr($type); ?>" class="<?php echo esc_attr($class); ?>" value="<?php echo empty($default)?'':$default; ?>" placeholder="<?php echo esc_attr($holder); ?>" type="text" /><?php
                        break;
                    }
                    case 'checkbox': { ?>
                        <input    data-name="<?php echo esc_attr($itemSlug); ?>" data-type="<?php echo esc_attr($type); ?>" class="<?php echo esc_attr($class); ?> hidden" value="<?php echo esc_attr($default); ?>" placeholder="<?php echo esc_attr($holder); ?>" type="checkbox" <?php echo esc_attr($default) ==='true'?'checked':''; ?> />
                        <div class="checkbox-text clearfix"><div class="checkbox-true"><?php _e('ON','ewebcraft'); ?></div><div class="checkbox-false"><?php _e('OFF','ewebcraft'); ?></div></div><?php
                        break;
                    }
                    case 'textArea': { ?>
                        <textarea data-name="<?php echo esc_attr($itemSlug); ?>" data-type="<?php echo esc_attr($type); ?>" class="<?php echo esc_attr($class); ?>" placeholder="<?php echo esc_attr($holder); ?>" data-tinyMCE="<?php echo wp_kses_post($tinyMCE); ?>" ><?php echo esc_attr($default); ?></textarea><?php
                        break;
                    }
                    case 'category':
                    case 'select': { ?>
                        ...
                        <div data-name="<?php echo esc_attr($itemSlug); ?>" data-type="<?php echo esc_attr($type); ?>" data-container-type="<?php echo esc_attr($container_type); ?>" class="<?php echo esc_attr($class); ?> container" placeholder="<?php echo esc_attr($holder); ?>" data-title-as="<?php echo esc_attr($title_as); ?>" data-add-button="<?php echo esc_attr($add_button); ?>" ><?php
                            if(!empty($default)) {
                                foreach ($default as $data) { ?>
                                    <div class="container-item<?php echo esc_attr($container_type)==='image_slider'?' expanded':''; ?>">
                                        <div class="list clearfix">
                                            <div class="name"><?php if(isset($data[$title_as]['default'])){echo wp_kses_post($data[$title_as]['default']);} ?></div>
                                            <div class="actions">
...
if (!function_exists('pbGetItem')) {
    function pbGetItem($itemSlug, $itemNewData = array(),$all=true) {
        global $jw_pbHeadSettings, $jw_pbItems;
        $itemArray = $jw_pbItems[$itemSlug];
        $itemArray['size'] = empty($itemNewData['size']) ? $itemArray['size'] : pbTextToSize($itemNewData['size']);
        ob_start(); ?>
        <div class="action-container item <?php echo esc_attr($itemArray['size']); ?>" data-slug="<?php echo esc_attr($itemSlug); ?>"<?php if(isset($itemArray['min-size'])){echo ' data-min="'.$itemArray['min-size'].'"';} ?> data-help="<?php echo isset($itemArray['help'])?$itemArray['help']:''; ?>">
            <div class="thumb"><span class="<?php echo esc_attr($itemSlug); ?>"></span><?php echo esc_attr($itemArray['name']); ?></div><?php
            if($all){ ?>
                <div class="list clearfix">
                    ...
                    <div class="name"><?php echo esc_attr($itemArray['name']); ?></div>
                    <div class="actions">
                ...
                <div class="data">
                    <div class="general-field-container"><?php
                        foreach ($jw_pbHeadSettings as $pbHeadSettingSlug => $pbHeadSetting) {
...
function pbGetRowTools($rowNewData=array()) {
    ...
    $output.='<div class="data">';
        $output.='<div class="custom-field-container">';
            foreach ($jw_pbRowSettings as $pbRowSettingSlug => $pbRowSetting){
                $pbRowSetting['default'] = isset($rowNewData[$pbRowSettingSlug]) ? $rowNewData[$pbRowSettingSlug] : '';
                ob_start();
    ...
    function pbTemplateAdd() {
        if (isset($_REQUEST['template_name']) && isset($_REQUEST['template_content'])) {
            ...
            if (isset($templates_array[$_REQUEST['template_name']])) {
                $response .= '<div class="error">' . __('Template name is already exist. Please insert the template name and try again', 'ewebcraft') . '</div>';
            } else {
                $templates_array[$_REQUEST['template_name']] = 'true';
                update_option('jw_pb_'.strtolower(THEMENAME).'_templates', $templates_array);
                update_option('jw_pb_'.strtolower(THEMENAME).'_templates_'.$_REQUEST['template_name'], $_REQUEST['template_content']);
                
                $response .= '<div class="succes">' . __('Template added', 'ewebcraft') . '</div>';
            }
    ...
    function pbTemplateRemove() {
        if (isset($_REQUEST['template_name'])) {
            ...
            if (isset($templates_array[$_REQUEST['template_name']])) {
                ...
                update_option('jw_pb_'.strtolower(THEMENAME).'_templates', $templates_array);
                update_option('jw_pb_'.strtolower(THEMENAME).'_templates_'.$_REQUEST['template_name'], '');
            } else {
                $response .= '<div class="error">' . __('Template name not exsist', 'ewebcraft') . '</div>';
            }
            die('<div class="response">' . $response . '</div>');
...
if (!function_exists('pbGetFontawesome') && is_user_logged_in()) {
    function pbGetFontawesome() {
        require_once (THEME_PATH . "/inc/pagebuilder/font-awesome.php");
        die();
    }
...
if (!function_exists('pbGetCircleChart') && is_user_logged_in()) {
    function pbGetCircleChart() {
        require_once (THEME_PATH . "/inc/pagebuilder/circle-chart.php");
        die();
    }
