        ...
        <tr id="field_<?php echo esc_attr($settings['id']); ?>">
            <th>
                <label for="<?php echo esc_attr($settings['id']); ?>">
                    <strong><?php echo esc_attr($settings['name']); ?></strong>
                    <span><?php echo esc_attr($settings['desc']); ?></span>
                </label>
            </th>
            <td>
...
if (!function_exists('settings_textarea')) {
    function settings_textarea($settings){ ?>
        <tr id="<?php echo esc_attr($settings['id']); ?>">
            ...
            <td>
                <textarea rows="5" name="<?php echo esc_attr($settings['id']); ?>"><?php echo esc_attr($settings['value']); ?></textarea>
            </td>
        </tr><?php
    }
}
    ...
    function settings_file($settings){ ?>
        <tr id="<?php echo esc_attr($settings['id']); ?>">
            ...
            <td>
                ...
                <a href="#" class="button insert-images theme_button format" onclick="browseimage('<?php echo esc_attr($settings['id']); ?>')"><?php _e('Insert image', "designinvento"); ?></a>
            </td>
        </tr><?php
    }
}
if (!function_exists('settings_selectbox')) {
    function settings_selectbox($settings){
        ...
        <tr id="<?php echo esc_attr($settings['id']); ?>">
            ...
            <td>
                <select class="selectbox" name="<?php echo esc_attr($settings['id']); ?>" data-value="<?php print $settings['value'];?>"><?php
                    foreach ($settings['options'] as $meta) {
                        echo '<option value="' . $meta . '" ';
                        echo esc_attr($meta) == $settings['value'] ? 'selected ' : '';
                        echo '>' . $meta . '</option>';
                    } ?>
                </select>
            </td>
...
if (!function_exists('settings_mselectbox')) {
    function settings_mselectbox($settings){
        $settings['options'] = array('Top', 'Header bottom', 'Top Transparent'); ?>
        <tr id="<?php echo esc_attr($settings['id']); ?>">
            <th>
                <label for="<?php echo esc_attr($settings['id']); ?>">
                    <strong><?php echo esc_attr($settings['name']); ?></strong>
        ...
        <tr id="<?php echo esc_attr($settings['id']); ?>">
            ...
            <td>
                <div class="type_select add_item_medium">
                    <select class="medium" name="<?php echo esc_attr($settings['id']); ?>" data-value="<?php print $settings['value'];?>"><?php
...
if (!function_exists('settings_gallery')) {
    function settings_gallery($settings){
        ...
        $gallery_thumbs = '';
        $button_text = ($meta) ? __('Edit Gallery', 'designinvento') : __('Upload Images', 'designinvento');
        if( $meta ) {
            $thumbs = explode(',', $meta);
        ...
        <tr id="<?php //echo esc_attr($settings['id']); ?>">
            ...
            <td>
                ...
                <input type="hidden" name="gallery_image_ids" id="gallery_image_ids" value="<?php echo esc_attr($meta) ? $meta : 'false'; ?>" />
                ...
                <?php $menus = get_terms( 'nav_menu', array( 'hide_empty' => false ) );
                        if ( !$menus ) {
                            ...
                            echo '<select name="'.$settings['id'].'" data-value="'.$settings['value'].'">';
									echo '<option value="">'. __('Default', 'designinvento') . '</option>';
