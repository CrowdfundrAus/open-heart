...
class flickrwidget extends WP_Widget {
    ...
    function flickrwidget() {
        ...
        parent::__construct(false, 'designinvento flickr', $widget_ops, $control_ops);
    }
    ...
    function form($instance) {
        ...
        ?>
        <p><label for="<?php echo esc_attr($this->get_field_id('flickr_title')); ?>"><?php _e('Title:', 'designinvento'); ?></label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('flickr_title')); ?>" name="<?php echo esc_attr($this->get_field_name('flickr_title')); ?>" type="text" value="<?php echo esc_attr($flickr_title); ?>" /></p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('flickr_type')); ?>">Type (user or group):</label>
            <select id="<?php echo esc_attr($this->get_field_id('flickr_type')); ?>" name="<?php echo esc_attr($this->get_field_name('flickr_type')); ?>" class="widefat">	
        ...
        <p><label for="<?php echo esc_attr($this->get_field_id('flickr_userid')); ?>"><?php _e('Flickr user ID:', 'designinvento'); ?></label>
            <br>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('flickr_userid')); ?>" name="<?php echo esc_attr($this->get_field_name('flickr_userid')); ?>" type="text" value="<?php echo esc_attr($flickr_userid); ?>" />
            <br>
            <small><em>Find ID <a href="http://idgettr.com/" target="_blank">http://idgettr.com/</a></em></small>
        </p>

        <p><label for="<?php echo esc_attr($this->get_field_id('flickr_num')); ?>"><?php _e('How many pictures display:', 'designinvento'); ?></label>
            <input maxlength="3" class="widefat" id="<?php echo esc_attr($this->get_field_id('flickr_num')); ?>" name="<?php echo esc_attr($this->get_field_name('flickr_num')); ?>" type="text" value="<?php echo esc_attr($flickr_num); ?>" />
        </p>
        <?php
    }
    ...
    function widget($args, $instance) {
        ...
        if (!empty($flickr_title)) {
            echo wp_kses_post($before_title . $flickr_title . $after_title);
        }
        echo '<div class="flickr-widget">';
            echo '<div class="flickr-channel">'; ?>
                <script type="text/javascript" src="http://www.flickr.com/badge_code_v2.gne?count=<?php echo esc_attr($flickr_num) ?>&amp;size=s&amp;layout=v&amp;source=<?php echo esc_attr($flickr_type) ?>&amp;<?php echo esc_attr($flickr_type) ?>=<?php echo esc_attr($flickr_userid) ?>"></script><?php
            echo '</div>';
        echo '</div>';
