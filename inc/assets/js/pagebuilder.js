        ...
        if($parent.css('position')==='relative'){
            ...
            jQuery( '<div id="pb-modal-container" class="'+$parent.attr('data-slug')+'" />' ).append(html).dialog({
                ...
                open: function(event, ui){
                    jQuery(this).closest('.ui-dialog').addClass('jw-pb-main-container');
...
function pbModalInitActions($currentModal){
    ...
    jQuery('[data-name="cc_icon"]',$currentModal).each(function(){
        ...
        jQuery('[data-name="cc_icon"]',$currCcFields).unbind('input').bind('input',function(){
            ...
            $html +='<div class="jw-circle-chart" data-percent="'+jQuery('[data-name="cc_percent"]',$currCcFields).val()+'">';
            ...
            jQuery('.jw-circle-chart',$currCc).easyPieChart({                
...
function pbModalContentEditor($id,$currentEditor){
    ...
    $currentEditor.hide().closest('.field-data').append(
        '<div class="wp-editor-tools jw-editor-menu hide-if-no-js">'+
        ...
        '</div>'+
        '<div id="wp-content-editor-container" class="jw-editor-content">'+
            $newEditorDiv.html()+
        '</div>'
    ).promise().done(function(){
        tinymce.init({
            ...
            external_plugins:{"twshortcodegenerator":$homeURI+"/wp-content/themes/open-heart/inc/assets/js/shortcode.js"},
            content_css:$homeURI+"/wp-includes/css/dashicons.min.css?ver=3.9,"+$homeURI+"/wp-includes/js/mediaelement/mediaelementplayer.min.css?ver=3.9,"+$homeURI+"/wp-includes/js/mediaelement/wp-mediaelement.css?ver=3.9,"+$homeURI+"/wp-includes/js/tinymce/skins/wordpress/wp-content.css?ver=3.9",
            selector:$id,
            resize:false,
            convert_urls:false,
            menubar:false,
...
function twSortDragg($container,$items,$placeholder,$connectWith){
    ...
    jQuery($container).sortable({
        ...
        start: function(event, ui) {
            var plus;
            if(ui.item.hasClass('size-1-6')) plus = 'size-1-6';
			else if(ui.item.hasClass('size-1-5')) plus = 'size-1-5';
			else if(ui.item.hasClass('size-1-4')) plus = 'size-1-4';
